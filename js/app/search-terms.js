define(["app/utils", "moment", "d3-context-menu", "ion.rangeSlider"], function(utils, moment, context, rangeSlide) {
	...
	visualization.display = function(history) {
		...
		d3.select("#title").append("h1").text("What are you focusing on?").attr("id", "viz_title");
		d3.select("#title").append("h2").text(uniqueTerms.length + " focusing tags in resently a week. Left click a tag for more options.");
		...
		function draw(words) {
			...
			.on("mouseover", function(d) {
				tooltip.text("Search tag indluding " + d.size + "urls, -- Left-click for more options");
